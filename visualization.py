...
def plot_with_lrl(positions, e_vec):
    plt.figure()
    plt.plot(positions[:,0], positions[:,1], label="Orbit")
    plt.quiver(0, 0, e_vec[0], e_vec[1], 
               angles='xy', scale_units='xy', scale=1, color='red', label='LRL vector')
    plt.scatter([0],[0])
    plt.axis("equal")
    plt.legend()
    plt.show()

def plot_vis_viva_phi(phi, v_vals):
    plt.figure()
    plt.plot(phi, v_vals)
    plt.xlabel("phi")
    plt.ylabel("v(phi)")
